         ...
         {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={280}>
               <PieChart>
                  ...
                  <Tooltip
                     formatter={(v, n) => [`${Number(v).toLocaleString()} students`, String(n)]}
                  />
                  <Legend
      ...
      <div className="chart-wrap">
         ...
         {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
               <PieChart>
                  <Pie data={data} cx="50%" cy="50%" outerRadius={90} dataKey="value" paddingAngle={4} label={({ name, percent }) => `${name} ${((percent ?? 0) * 100).toFixed(0)}%`} labelLine={false}>
                     <Cell fill={C.primary} />
                     <Cell fill={C.accent} />
                  </Pie>
                  <Tooltip />
