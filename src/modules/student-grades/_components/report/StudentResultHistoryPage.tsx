...
import { buildReportStudentInfo, calculateReportSummary, formatSemesterLabel, toReportCourses } from "./utils";
...
export default function StudentResultHistoryPage() {
   ...
   const canDownload = Boolean(studentInfo && selectedAcademicYear && selectedSemester && reportCourses.length > 0);
      ...
      <PermissionGate
         ...
         fallback={
            <div className="flex flex-col items-center justify-center gap-3 py-24 text-muted-foreground">
               ...
               <p className="text-sm">You do not have permission to view your grade report.</p>
            </div>
         }
      >
            ...
            <section className="overflow-hidden rounded-3xl border border-primary/20 bg-linear-to-br from-primary/15 via-background to-amber-500/10 p-6">
               <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                     ...
                     <h1 className="mt-2 text-2xl font-bold text-foreground sm:text-3xl">Student Grade Report</h1>
                     <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                        Review your published semester results, breakdown by course, and export a branded official slip.
                     </p>
                  </div>

                  <Button onClick={handleDownload} disabled={!canDownload || isDownloading} className="gap-2">
                     <Download size={16} />
