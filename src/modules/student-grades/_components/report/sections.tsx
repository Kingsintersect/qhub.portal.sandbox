import { Award, BarChart3, Calendar, FileText, GraduationCap, TrendingUp } from "lucide-react";
         ...
         <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-4">
               <div className="rounded-2xl bg-white p-2 shadow-sm">
                  <img src={institutionLogoUrl} alt={`${institutionName} logo`} className="h-14 w-14 object-contain" />
               </div>
               <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-white/70">Official Result Slip</p>
                  <h1 className="mt-1 text-2xl font-bold">Student Grade Report</h1>
                  <p className="mt-1 text-sm text-white/80">
                     {formatSemesterLabel(semester)} · {academicYear}
                  </p>
                  <p className="text-xs text-white/60">{institutionName} 5.00 grading system</p>
               </div>
            </div>

            <div className="rounded-2xl bg-white/10 p-4 text-right backdrop-blur-sm">
               <p className="text-xs uppercase tracking-[0.16em] text-white/65">Current GPA</p>
               <p className="mt-1 text-3xl font-bold text-amber-300">{summary.gpa.toFixed(2)}</p>
               <p className="text-sm font-medium text-emerald-200">{summary.degreeClass}</p>
      ...
      <div className="border-b border-slate-200 px-6 py-6">
         <div className="flex flex-col gap-6 md:flex-row md:items-center">
            <div className="flex w-full flex-col items-center gap-3 md:w-52">
               {student.avatarUrl ? (
                  <div className="h-24 w-24 overflow-hidden rounded-full border-4 border-slate-200 shadow-sm">
               ...
               <h2 className="text-center text-lg font-bold uppercase tracking-wide text-[#750303]">
                  {student.fullName}
               </h2>
            </div>
               ...
               <tbody className="divide-y divide-slate-200 bg-white">
                  {courses.map((course) => (
                     <tr key={course.id} className="transition-colors hover:bg-slate-50">
                        <td className="px-4 py-4 font-semibold text-slate-900">{course.courseCode}</td>
                        <td className="px-4 py-4 text-slate-900">{course.courseTitle}</td>
                        <td className="px-4 py-4 text-center font-medium text-slate-900">{course.creditLoad}</td>
                        <td className="px-4 py-4 text-center font-medium text-slate-900">{course.score.toFixed(0)}%</td>
                        <td className="px-4 py-4 text-center">
                           <span className="inline-flex rounded-full border border-slate-200 px-3 py-1 text-sm font-semibold text-slate-900">
                              {course.grade}
                           </span>
                        </td>
                        <td className="px-4 py-4 text-center font-medium text-slate-900">{course.gradePoint.toFixed(2)}</td>
                        <td className="px-4 py-4 text-center font-semibold text-blue-600">{course.qualityPoints.toFixed(2)}</td>
                     </tr>
                  ))}
   ...
   return (
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
         <div className="mb-4 flex items-center gap-2">
            ...
            <h4 className="font-semibold text-slate-800">Grade Distribution Analysis</h4>
         </div>

         <div className="space-y-3">
            {summary.gradeDistribution.map((item) => (
               <div key={item.grade} className="flex items-center gap-3">
                  <div className="w-8 text-center font-bold text-slate-700">{item.grade}</div>
         ...
         <div className="mt-4 border-t border-slate-200 pt-4 text-sm text-slate-600">
            <div className="flex justify-between">
               ...
               <span className="font-medium text-slate-900">{summary.gradeDistribution.reduce((sum, item) => sum + item.count, 0)}</span>
            </div>
         </div>
      </div>
   );
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
         ...
         <div className="space-y-4">
            <div className="rounded-2xl bg-slate-50 p-4 text-center">
               <div className="text-3xl font-bold text-slate-800">{summary.gpa.toFixed(2)}</div>
               <div className="text-sm text-slate-600">Current GPA</div>
               <div className={cn("mt-2 inline-flex rounded-full px-3 py-1 text-sm font-medium", summary.academicStanding.color, summary.academicStanding.bgColor.replace("bg-", "bg-") + "/15") }>
                  {summary.degreeClass}
               </div>
            </div>
            ...
            <div>
               ...
               <div className="h-3 w-full rounded-full bg-slate-200">
                  <div className={cn("h-3 rounded-full", summary.academicStanding.bgColor)} style={{ width: `${progressPercentage}%` }} />
               </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-slate-600">
               <ScaleLine left="4.50-5.00" right="First Class" color="text-emerald-600" />
               <ScaleLine left="1.50-2.39" right="Third Class" color="text-amber-600" />
               <ScaleLine left="3.50-4.49" right="2nd Class Upper" color="text-blue-600" />
               <ScaleLine left="1.00-1.49" right="Pass" color="text-slate-600" />
               <ScaleLine left="2.40-3.49" right="2nd Class Lower" color="text-violet-600" />
            ...
            {nextTarget && (
               <div className="rounded-2xl bg-blue-50 p-3">
                  ...
                  <div className="mt-1 text-xs text-blue-600">Gap: {(nextTarget.target - summary.gpa).toFixed(2)} points</div>
               </div>
            )}
         </div>
      </div>
...
function ScaleLine({ left, right, color }: { left: string; right: string; color: string }) {
   return (
      ...
      <div className="rounded-b-3xl border-t border-slate-200 bg-slate-100 px-6 py-6">
         <div className="mx-auto max-w-4xl">
            <div className="mb-3 flex items-center justify-center gap-2">
               ...
               <h4 className="text-sm font-medium text-slate-700">Official Grade Report</h4>
            </div>

            <div className="space-y-2 text-center">
               <p className="text-sm text-slate-600">
                  This is an official academic transcript for <strong>{formatSemesterLabel(semester)}</strong> of the <strong>{academicYear}</strong> academic session at <strong>{institutionName}</strong>.
               </p>
               <p className="text-sm text-slate-600">Computed using the Nigerian University 5.00 Grade Point System.</p>
