...
export function PublishResultsTable({ grades, canPublish = false }: PublishResultsTableProps) {  // ← ADD THIS
   ...
   const filtered = useMemo(() => {
      ...
      if (feeFilter === "clear") list = list.filter((g) => !g.hasOutstandingFees);      // ← uses feeFilter
      if (feeFilter === "outstanding") list = list.filter((g) => g.hasOutstandingFees); // ← uses feeFilter
      if (sortKey) {
         list = [...list].sort((a, b) => {
            const av = a[sortKey] ?? "";
            const bv = b[sortKey] ?? "";
      ...
      return list;
   }, [grades, search, statusFilter, sortKey, sortDir, feeFilter]);
   ...
   return (
      <div className="space-y-3">
         ...
         {/* Select-all banner - only show if user can publish */}
         {canPublish && someSelected && (
            <AnimatePresence>
               {someSelected && (
                  <motion.div
                  ...
                  <table className="w-full text-xs">
                     <thead>
                        <tr className="border-b border-border bg-muted/40">
                           {/* Checkbox column - only show if user can publish */}
                           {canPublish && (
                     ...
                     <tbody>
                        {pageSlice.map((grade, i) => {
                           ...
                           return (
                              ...
                              >
                                 {/* Checkbox cell - only show if user can publish */}
                                 {canPublish && (
