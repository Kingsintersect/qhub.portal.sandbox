...
export function useMyTimetable(params?: { semesterId?: number; groupByDay?: boolean }) {
...
export function useStudentTimetable(studentId: string, params?: { semesterId?: number }) {
    return useQuery({
...
export function useAllSchedules(filters?: ScheduleFilter) {
    return useQuery({ ...timetableQueryOptions.admin(filters), staleTime: 5 * 60 * 1000 });
}
...
export function useVenues() {
    return useQuery({ ...timetableQueryOptions.venues(), staleTime: 10 * 60 * 1000 });
}

export function useVenueAvailability(
    params: { venue: string; dayOfWeek: string; semesterId: number; excludeScheduleId?: number } | null
) {
    return useQuery({
        ...timetableQueryOptions.venueAvailability(params as Parameters<typeof timetableQueryOptions.venueAvailability>[0]),
