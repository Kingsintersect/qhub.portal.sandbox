...
export function VenueAvailabilityChecker() {
   const [query, setQuery] = useState<{ venue: string; dayOfWeek: string } | null>(null);
   ...
   const { data, isLoading } = useVenueAvailability(
      query ? { venue: query.venue, dayOfWeek: query.dayOfWeek, semesterId: 1 } : null
   );
      ...
      <div className="space-y-6">
         ...
         <form onSubmit={handleSubmit(onSubmit)} className="flex flex-wrap items-end gap-3">
            ...
            <div className="space-y-1">
               ...
               <Select onValueChange={(val) => setValue("dayOfWeek", val as typeof DAYS[0])}>
            ...
            <Button type="submit" disabled={isLoading} className="gap-2">
               {isLoading ? <Loader2 size={14} className="animate-spin" /> : <Building size={14} />}
               Check
            </Button>
         </form>
         ...
         {query && (
            <div className="space-y-4">
               ...
               {data?.busySlots?.length ? (
                  <div className="space-y-1">
                     <p className="text-sm font-semibold text-foreground">Busy Slots</p>
               ...
               {data?.freeWindows?.length ? (
                  <div className="space-y-1">
                     <p className="text-sm font-semibold text-foreground">Free Windows</p>
                     <div className="flex flex-wrap gap-2">
                        {data.freeWindows.map((w: { startTime: string; endTime: string }, i: number) => (
                           <span key={i} className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-xs font-mono px-2 py-1 rounded-md">
               ...
               {data && !data.busySlots?.length && !data.freeWindows?.length && (
                  <p className="text-sm text-muted-foreground">No availability data found for <strong>{query.venue}</strong>.</p>
               )}
            </div>
         )}
      </div>
