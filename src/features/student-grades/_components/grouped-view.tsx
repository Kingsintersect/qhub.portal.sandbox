...
import { ChevronDown, Users, BarChart2, CheckCircle2, Eye, Pencil } from "lucide-react";
...
function GroupRow({ group, onViewGrade, canManage = false }: GroupRowProps) {  // ← ADD THIS
...
export function GradesGroupedView({ data, loading, onViewGrade, canManage = false }: GroupedViewProps) {  // ← ADD THIS
   if (loading) {
      return (
      ...
      <div className="space-y-3">
         {data.map((group) => (
            <GroupRow
               ...
               canManage={canManage}  // ← PASS IT DOWN
            />
         ))}
      </div>
   );
