...
function LogoInner({
  ...
  // Defaults resolve from the institution this host belongs to, not from a
  // build-time constant — the same <Logo /> renders each tenant's own
  // identity. Explicit props still win.
  const institutionName = useInstitutionName(UNIVERSITY_NAME)
  const institutionLogo = useInstitutionLogo(UNIVERSITY_LOGO_URL)

  const resolvedSrc = src ?? institutionLogo
  const resolvedTitle = title ?? institutionName
  const resolvedAlt = alt ?? `${institutionName} logo`
      ...
      {showText && (
        <div className={cn("min-w-0", textClassName)}>
          ...
          {subtitle && (
            <div
              className={cn(
                "text-xs leading-tight opacity-70",
                subtitleClassName
              )}
            >
              {subtitle}
