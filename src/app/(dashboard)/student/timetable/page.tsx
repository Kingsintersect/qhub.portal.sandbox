...
import type { DayOfWeek, TimetableSlot, ClassType } from "@/modules/timetable/types/timetable.types";
...
export default function MyTimetablePage() {
   ...
   const { data, isLoading } = useMyTimetable({ semesterId: effectiveSemesterId });
   ...
   const filteredSlots = useMemo(() => {
      ...
      return slots.filter((slot) => {
         ...
         const matchesClassType = !selectedClassType || slot.classType === selectedClassType;
         const matchesSearch =
            !search ||
   ...
   }, [slots, selectedDay, selectedClassType, searchTerm]);

   const uniqueCoursesCount = new Set(filteredSlots.map((slot) => slot.courseCode)).size;
         ...
         <div className="space-y-6">
            <section className="overflow-hidden rounded-3xl border border-primary/20 bg-linear-to-br from-primary/15 via-background to-cyan-500/10 p-6">
               <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                     <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Weekly Planner</p>
                     <h1 className="mt-2 text-2xl font-bold text-foreground sm:text-3xl">My Timetable</h1>
                     <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                        Plan classes, track lecture load, and quickly pivot between calendar and upcoming sessions.
                     </p>
                  </div>
               ...
               <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
                     ...
                     <p className="mt-1 text-2xl font-bold text-foreground">{filteredSlots.length}</p>
                  </div>
                  <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
                     <p className="text-xs text-muted-foreground">Distinct Courses</p>
                     <p className="mt-1 text-2xl font-bold text-foreground">{uniqueCoursesCount}</p>
                  </div>
                  <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
                     <p className="text-xs text-muted-foreground">Weekly Contact Time</p>
                     <p className="mt-1 text-2xl font-bold text-foreground">{totalHoursLabel}</p>
                  </div>
               </div>
            </section>
            ...
            <section className="rounded-3xl border border-border/70 bg-card/95 p-5 shadow-sm">
               <div className="mb-4 flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-foreground">Filter Timetable</p>
               </div>
               ...
               <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
                  <div>
                     ...
                     <Select
                        ...
                        onValueChange={(value) => setSelectedDay(value === "all" ? null : (value as DayOfWeek))}
                     >
                  ...
                  <div>
                     ...
                     <Select
                        ...
                        onValueChange={(value) =>
                           setSelectedClassType(value === "all" ? null : (value as ClassType))
                        }
                     >
                        <SelectTrigger className="w-full justify-between">
