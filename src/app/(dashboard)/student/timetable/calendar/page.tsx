...
import { ShieldOff, CalendarDays, Search, Filter } from "lucide-react";
...
export default function MyCalendarPage() {
    ...
    const [eventType, setEventType] = useState<"all" | "course" | "site" | "user" | "zoom">("all");
        ...
        >
            <div className="space-y-6">
                <section className="rounded-3xl border border-primary/20 bg-linear-to-br from-primary/15 via-background to-cyan-500/10 p-6">
                    <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Calendar Hub</p>
                    <h1 className="mt-2 text-2xl font-bold text-foreground sm:text-3xl">Calendar &amp; Events</h1>
                    <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                        Track academic events, live sessions, and announcements in one place.
                    </p>

                    <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
                        <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
                            <p className="text-xs text-muted-foreground">Visible Events</p>
                            <p className="mt-1 text-2xl font-bold text-foreground">{filteredEvents.length}</p>
                        </div>
                        <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
                            <p className="text-xs text-muted-foreground">Today</p>
                            <p className="mt-1 text-2xl font-bold text-foreground">{todayCount}</p>
                        </div>
                        <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
                            <p className="text-xs text-muted-foreground">Type Filter</p>
                            <p className="mt-1 text-2xl font-bold text-foreground">{eventType === "all" ? "All" : eventType}</p>
                        </div>
                    </div>
                </section>
                ...
                <section className="rounded-3xl border border-border/70 bg-card/95 p-5 shadow-sm">
                    <div className="mb-4 flex items-center gap-2">
                        ...
                        <p className="text-sm font-semibold text-foreground">Filter Events</p>
                    </div>
