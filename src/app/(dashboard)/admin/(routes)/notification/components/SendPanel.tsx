...
import { useNotificationTemplates, useSendNotification, useSendBulkNotification } from "@/hooks/useNotifications";
...
const singleSchema = z.object({
   userId: z.string().min(1, "Enter a user ID").regex(/^\d+$/, "Must be a valid number"),
...
function TemplateHint({ templates, onSelect }: {
   templates: { id: number; name: string; subject: string; body: string; channel: NotificationChannel }[];
   onSelect: (subject: string, body: string, channel: NotificationChannel) => void;
}) {
   ...
   return (
      <div className="space-y-1.5">
         ...
         <p className="text-xs text-muted-foreground">Selecting a template will pre-fill subject, body and channel.</p>
      </div>
   );
}
...
export function SendPanel() {
   ...
   const onSingleSubmit = async (data: SingleForm) => {
      try {
         ...
         toast.success(`Notification sent to User #${data.userId}`);
         resetSingle();
      } catch (err: unknown) {
   ...
   const onBulkSubmit = async (data: BulkForm) => {
      const userIds = data.userIds.split(/[\s,]+/).map(Number).filter(Boolean);
      if (userIds.length === 0) {
      ...
      try {
         ...
         toast.success(`Sent to ${res.sent} user(s)${res.errors.length ? `, ${res.errors.length} failed` : ""}`);
         resetBulk();
      } catch (err: unknown) {
         toast.error(err instanceof Error ? err.message : "Failed to send");
      }
   ...
   const fillSingleFromTemplate = (subject: string, body: string, channel: NotificationChannel) => {
      setSingle("subject", subject);
      setSingle("body", body);
      setSingle("channel", channel);
   };

   const fillBulkFromTemplate = (subject: string, body: string, channel: NotificationChannel) => {
      ...
      <div className="space-y-4">
         ...
         {!isBulk && (
            <SectionCard title="Send to Single User" icon={Send}>
               <div className="space-y-4 max-w-xl">
                  <TemplateHint templates={templates} onSelect={fillSingleFromTemplate} />
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                     <div className="space-y-1.5">
                        ...
                        <Input
                           ...
                           {...regSingle("userId")}
                        />
                        {singleErrors.userId && <p className="text-xs text-destructive">{singleErrors.userId.message}</p>}
                     </div>
                     <div className="space-y-1.5">
                        <Label>Channel</Label>
                        <Select
                           value={singleChannel}
                           onValueChange={(v) => setSingle("channel", v as NotificationChannel, { shouldValidate: true })}
                        >
                  ...
                  <div className="space-y-1.5">
                     ...
                     <Input
                        ...
                        {...regSingle("subject")}
                     />
                     {singleErrors.subject && <p className="text-xs text-destructive">{singleErrors.subject.message}</p>}
                  </div>
                  ...
                  <div className="space-y-1.5">
                     ...
                     <Textarea
                        ...
                        {...regSingle("body")}
                     />
                     {singleErrors.body && <p className="text-xs text-destructive">{singleErrors.body.message}</p>}
                  </div>
         ...
         {isBulk && (
            <SectionCard title="Bulk Send" icon={Users}>
               <div className="space-y-4 max-w-xl">
                  <TemplateHint templates={templates} onSelect={fillBulkFromTemplate} />
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                     <div className="space-y-1.5 sm:col-span-2">
                        ...
                        />
                        {bulkErrors.userIds
                           ? <p className="text-xs text-destructive">{bulkErrors.userIds.message}</p>
                           : <p className="text-xs text-muted-foreground">Comma or space separated user IDs</p>}
                     </div>
                     <div className="space-y-1.5">
                        <Label>Channel</Label>
                        <Select
                           value={bulkChannel}
                           onValueChange={(v) => setBulk("channel", v as NotificationChannel)}
                        >
                           <SelectTrigger>
                              <SelectValue />
                           </SelectTrigger>
                  ...
                  <div className="space-y-1.5">
                     ...
                     <Input
                        ...
                        {...regBulk("subject")}
                     />
                     {bulkErrors.subject && <p className="text-xs text-destructive">{bulkErrors.subject.message}</p>}
                  </div>
                  ...
                  <div className="space-y-1.5">
                     ...
                     <Textarea
                        ...
                        {...regBulk("body")}
                     />
                     {bulkErrors.body && <p className="text-xs text-destructive">{bulkErrors.body.message}</p>}
                  </div>
