...
import { useSettings, useCreateSetting, useUpdateSetting, useDeleteSetting } from "@/hooks/useConfiguration";
import type { Setting, SettingGroup, CreateSettingPayload } from "@/types/school";
...
export default function ConfigPage() {
   ...
   const filtered = useMemo(() => {
      let list = activeGroup === "all" ? allSettings : allSettings.filter((s) => s.group === activeGroup);
      if (search.trim()) {
   ...
   const handleFormSubmit = async (data: { key?: string; value: string; group: string }) => {
      try {
         if (formMode === "create") {
   ...
   // ── Active group label ──────────────────────
   const activeLabel = SETTING_GROUPS.find((g) => g.value === activeGroup)?.label ?? "Settings";

   return (
      <div className="mx-auto px-4 py-8 sm:px-6 lg:px-8 space-y-6">
         {/* Page header */}
         <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
               ...
               <p className="mt-1 text-sm text-muted-foreground">
                  Manage key-value settings for university, academic, payment and system behaviour.
               </p>
            </div>
         ...
         {/* Group tabs */}
         <GroupTabs active={activeGroup} onChange={setActiveGroup} counts={groupCounts} />
