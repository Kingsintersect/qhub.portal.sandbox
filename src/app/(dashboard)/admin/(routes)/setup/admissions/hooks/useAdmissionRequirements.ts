...
interface AdmissionRequirementsResponse {
...
export function useAdmissionRequirements(cycleId: string | null) {
    return useQuery<AdmissionRequirementsResponse, Error, AdmissionRequirement[]>({
        queryKey: admissionRequirementKeys.byCycle(cycleId!),
        queryFn: () => dummyAdmissionRequirementApi.listByCycle(cycleId!),
        select: (res): AdmissionRequirement[] => res.data,
        enabled: !!cycleId,
    });
}
